import { find, get, includes, isEqual, setWith } from "lodash";
...
const StepFour = ({ id = null, ...props }) => {
  ...
  const setProduct = useSettingsStore((state) =>
    get(state, "setProduct", () => {})
  );
  const resetProduct = useSettingsStore((state) =>
    get(state, "resetProduct", () => {})
  );
  const resetRiskList = useSettingsStore((state) =>
    get(state, "resetRiskList", [])
  );
  ...
  let { data: risks } = useGetAllQuery({
    key: KEYS.risk,
    url: `${URLS.risk}/list`,
  });
  let risksOptions = getSelectOptionsListFromData(
    get(risks, `data.data`, []),
    "_id",
    "name"
  );

  let { data: franchises } = useGetAllQuery({
    key: KEYS.typeoffranchise,
    url: `${URLS.typeoffranchise}/list`,
  });
  franchises = getSelectOptionsListFromData(
    get(franchises, `data.data`, []),
    "_id",
    "name"
  );

  let { data: baseFranchises } = useGetAllQuery({
    key: KEYS.baseoffranchise,
    url: `${URLS.baseoffranchise}/list`,
  });
  baseFranchises = getSelectOptionsListFromData(
    get(baseFranchises, `data.data`, []),
    "_id",
    "name"
  );
  ...
  const setFieldValue = (value, name = "") => {
    ...
    if (includes(["tariff.allowAgreement", "tariff.limitOfAgreement"], name)) {
      setTarif((prev) => ({ ...prev, [name]: value }));
    }

    if (includes(name, "tariff.tariffPerClass")) {
              ...
              <Col xs={12} className={"horizontal-scroll"}>
                ...
                <Table
                  ...
                  thead={[
                    ...
                    "Укажите тип франшизы",
                    "Укажите базу франшизы",
                    "Франшиза",
                  ]}
                >
                  {get(product, "riskData", []).map((item, i) => (
                      ...
                      <td>
                        <Flex justify={"center"}>
                          <Field
                            ...
                            property={{ hideLabel: true }}
                            disabled={
                              !get(otherParams, `franchise[${i}].hasFranchise`)
                            }
                          />
                        </Flex>
                      </td>
                      ...
                      <td className={"w-250"}>
                        <Flex justify={"center"}>
                          <Field
                            ...
                            options={franchises}
                            isDisabled={
                              !!!get(fields, `franchise[${i}].hasFranchise`)
                            }
                            defaultValue={get(
                              product,
                              `franchise[${i}].franchiseType`
                            )}
                          />
                        </Flex>
                      </td>
                          ...
                          <Field
                            ...
                            property={{
                              ...
                              disabled: !(
                                ...
                                !get(otherParams, `franchise[${i}].isFixed`)
                              ),
                            }}
                            defaultValue={get(
                              product,
