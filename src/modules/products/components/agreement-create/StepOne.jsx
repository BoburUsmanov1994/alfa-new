...
const StepOne = ({ id = null, ...props }) => {
  ...
  const setAgreement = useSettingsStore((state) =>
    get(state, "setAgreement", () => {})
  );
  const setInsurer = useSettingsStore((state) =>
    get(state, "setInsurer", () => {})
  );
  const resetInsurer = useSettingsStore((state) =>
    get(state, "resetInsurer", () => {})
  );
  const setBeneficiary = useSettingsStore((state) =>
    get(state, "setBeneficiary", () => {})
  );
  const resetBeneficiary = useSettingsStore((state) =>
    get(state, "resetBeneficiary", () => {})
  );
  const setPledger = useSettingsStore((state) =>
    get(state, "setPledger", () => {})
  );
  const addPledgers = useSettingsStore((state) =>
    get(state, "addPledgers", () => {})
  );
  const resetAgreement = useSettingsStore((state) =>
    get(state, "resetAgreement", () => {})
  );
  const resetPledger = useSettingsStore((state) =>
    get(state, "resetPledger", () => {})
  );
  const removePledgers = useSettingsStore((state) =>
    get(state, "removePledgers", () => {})
  );
  const resetPledgers = useSettingsStore((state) =>
    get(state, "resetPledgers", () => {})
  );
  const removeObjects = useSettingsStore((state) =>
    get(state, "removeObjects", () => {})
  );
  ...
  const beneficiary = useSettingsStore((state) =>
    get(state, "beneficiary", {})
  );
  ...
  groups = getSelectOptionsListFromData(
    get(groups, `data.data`, []),
    "_id",
    "name"
  );
  ...
  subGroups = getSelectOptionsListFromData(
    get(subGroups, `data.data`, []),
    "_id",
    "name"
  );
  ...
  const productsList = getSelectOptionsListFromData(
  ...
  const setRisk = (value, name) => {
    if (isEqual(name, "product")) {
      setProduct(
        find(get(products, "data.data", []), (p) =>
          isEqual(get(p, "_id"), value)
        )
      );
    }
  };
      ...
      },
      {
        onSuccess: (data) => {
          if (isEqual(type, "insurer")) {
            if (
              isEqual(get(insurer, "type"), PERSON_TYPE.person) &&
            ...
            if (
              isEqual(get(insurer, "type"), PERSON_TYPE.organization) &&
            ...
            if (
              isEqual(get(beneficiary, "type"), PERSON_TYPE.person) &&
              get(data, "data.person")
            ) {
              setBeneficiary({
                ...beneficiary,
            ...
            if (
              isEqual(get(beneficiary, "type"), PERSON_TYPE.organization) &&
              get(data, "data.organization")
            ) {
              setBeneficiary({
                ...beneficiary,
            ...
            if (
              isEqual(get(pledger, "type"), PERSON_TYPE.person) &&
              get(data, "data.person")
            ) {
              setPledger({
                ...pledger,
            ...
            if (
              isEqual(get(pledger, "type"), PERSON_TYPE.organization) &&
              get(data, "data.organization")
            ) {
              setPledger({
                ...pledger,
      ...
      <Col xs={12}>
        <Form formRequest={nextStep} getValueFromField={setRisk}>
          ...
          <Row>
                ...
                {get(product, "risk", [])?.length > 0 && (
                  <Col xs={12}>
                    ...
                    <Table
                      ...
                      {get(product, "risk", [])?.map((item, i) => (
                        <tr key={i + 1}>
                          <td>{get(item, "riskType.name")}</td>
                          <td>{get(item, "name")}</td>
                          <td>{get(item, "insuranceClass.name")}</td>
                        </tr>
                      ))}
                    </Table>
                  </Col>
              ...
              <Row>
                <Col xs={12}>
                  <Flex className={"mb-15"}>
                    <Button
                      type={"button"}
                      gray={isEqual(get(insurer, "type"), PERSON_TYPE.person)}
                      transparent={
                        !isEqual(get(insurer, "type"), PERSON_TYPE.person)
                      }
                      onClick={() =>
                  ...
                  <Flex>
                    <Field
                      ...
                      defaultValue={`${get(
                        ...
                        ""
                      )} ${get(insurer, "data.fullName.firstname", "")} ${get(
                        insurer,
                        "data.name",
                        ""
                      )}`}
                    ...
                    <Button
                      type={"button"}
                      onClick={() =>
                        setInsurer({ ...insurer, openModal: true })
                      }
            ...
            {get(product, "hasBeneficary", false) && (
              <>
                <Col xs={12} className={"mb-15"}>
                      ...
                      <Flex>
                        <Field
                          ...
                          defaultValue={`${get(
                            ...
                            ""
                          )} ${get(beneficiary, "data.name", "")}`}
                        />
                        <Button
                          type={"button"}
                          onClick={() =>
                            setBeneficiary({ ...beneficiary, openModal: true })
                          }
                          className={"mb-25 ml-15"}
                          success>
                          {t("Выбрать")}
              ...
              <Row>
                <Col xs={12}>
                  <Flex className={"mb-15"}>
                    <Button
                      type={"button"}
                      gray={isEqual(get(pledger, "type"), PERSON_TYPE.person)}
                      transparent={
                        !isEqual(get(pledger, "type"), PERSON_TYPE.person)
                      }
                      onClick={() =>
                  ...
                  <Flex>
                    <Field
                      ...
                      defaultValue={`${get(
                        ...
                        "data.fullName.lastname",
                        ""
                      )} ${get(pledger, "data.fullName.firstname", "")} ${get(
                        pledger,
                        "data.name",
                        ""
                      )}`}
                    ...
                    <Table
                      ...
                      {pledgers.map((item, i) => (
                        <tr key={i + 1}>
                          <td>{get(item, "type")}</td>
                          <td>
                            {`${get(item, "fullName.lastname", "")} ${get(
                              item,
                              "fullName.firstname",
                              ""
                            )} ${get(item, "name", "")}`}
                          ...
                          <td
                            ...
                            onClick={() =>
                              removePledgers(get(item, "id", null))
                            }>
                            <Trash2 color={"#dc2626"} />
                          </td>
                        </tr>
        ...
        <Modal
          ...
          visible={get(insurer, "openModal", false)}
          ...
          <Button
            ...
            onClick={() =>
              setInsurer({ ...insurer, type: PERSON_TYPE.person })
            }>
            {t("Физическое лицо")}
          </Button>
          <Button
            ...
            onClick={() =>
              setInsurer({ ...insurer, type: PERSON_TYPE.organization })
            }>
            {t("Юридическое лицо")}
          </Button>
          <Form
            formRequest={(data) => agentFilter(data, "insurer")}
            footer={
              <>
        ...
        <Modal
          title={t("Выберите тип выгодоприобритатель")}
          visible={get(beneficiary, "openModal", false)}
          ...
          <Button
            ...
            onClick={() =>
              setBeneficiary({ ...beneficiary, type: PERSON_TYPE.person })
            }>
            {t("Физическое лицо")}
          </Button>
          <Button
            ...
            onClick={() =>
              setBeneficiary({ ...beneficiary, type: PERSON_TYPE.organization })
            }>
            {t("Юридическое лицо")}
          </Button>
          <Form
            formRequest={(data) => agentFilter(data, "beneficiary")}
            footer={
              <>
        ...
        <Modal
          ...
          visible={get(pledger, "openModal", false)}
          hide={(val) => setPledger({ ...pledger, openModal: val })}>
          {filterLoading && <ContentLoader />}
          <Button
            type={"button"}
            className={"mt-15"}
            yellow={isEqual(get(pledger, "type"), "physical")}
            transparent={!isEqual(get(pledger, "type"), PERSON_TYPE.person)}
            onClick={() =>
              setPledger({ ...pledger, type: PERSON_TYPE.person })
            }>
            {t("Физическое лицо")}
          </Button>
          <Button
            type={"button"}
            className={"ml-15"}
            yellow={isEqual(get(pledger, "type"), "juridical")}
            transparent={
              !isEqual(get(pledger, "type"), PERSON_TYPE.organization)
            }
            onClick={() =>
              setPledger({ ...pledger, type: PERSON_TYPE.organization })
            }>
            {t("Юридическое лицо")}
          </Button>
          <Form
            formRequest={(data) => agentFilter(data, "pledger")}
            footer={
              <>
                <Button>{t("Find")}</Button>
                <Button
