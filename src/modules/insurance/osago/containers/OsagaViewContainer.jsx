...
const ViewContainer = ({application_number = null}) => {
    ...
    const breadcrumbs = useMemo(() => [{
        id: 1, title: 'ОСАГО', path: '/insurance/osago',
    }, {
        id: 2, title: 'ОСАГО', path: '/insurance/osago',
    }], [])
    ...
    const {data: insuranceTerms, isLoading: isLoadingInsuranceTerms} = useGetAllQuery({
        key: KEYS.osagoInsuranceTerms, url: `${URLS.osagoInsuranceTerms}`
    })
    const insuranceTermsList = getSelectOptionsListFromData(get(insuranceTerms, `data`, []), 'id', 'name')

    const {data: termCategory, isLoading: isLoadingTermCategory} = useGetAllQuery({
        key: KEYS.termCategories, url: URLS.termCategories
    })
    const termCategoryList = getSelectOptionsListFromData(get(termCategory, `data`, []), 'id', 'name')

    const {data: accidentType, isLoading: isLoadingAccidentType} = useGetAllQuery({
        key: KEYS.accidentTypes, url: URLS.accidentTypes
    })
    const accidentTypeList = getSelectOptionsListFromData(get(accidentType, `data`, []), 'id', 'name')

    const {data: discounts, isLoading: isLoadingDiscount} = useGetAllQuery({
        key: KEYS.discounts, url: URLS.discounts
    })
    const discountList = getSelectOptionsListFromData(get(discounts, `data`, []), 'id', 'name')


    const {data: region, isLoading: isLoadingRegion} = useGetAllQuery({
        key: KEYS.osagoRegions, url: URLS.osagoRegions
    })
    const regionList = getSelectOptionsListFromData(get(region, `data`, []), 'id', 'name')

    const {data: genders} = useGetAllQuery({
        key: KEYS.osagoGenders, url: URLS.osagoGenders
    })
    const genderList = getSelectOptionsListFromData(get(genders, `data`, []), 'id', 'name')

    const {data: residentTypes} = useGetAllQuery({
        key: KEYS.osagoResidentTypes, url: URLS.osagoResidentTypes
    })
    const residentTypeList = getSelectOptionsListFromData(get(residentTypes, `data`, []), 'id', 'name')


    const {data: vehicleTypes} = useGetAllQuery({
        key: KEYS.osagoVehicleTypes, url: URLS.osagoVehicleTypes
    })
    const vehicleTypeList = getSelectOptionsListFromData(get(vehicleTypes, `data`, []), 'id', 'name')

    const {data: driverTypes} = useGetAllQuery({
        key: KEYS.driverTypes, url: URLS.driverTypes
    })
    const driverTypeList = getSelectOptionsListFromData(get(driverTypes, `data`, []), 'id', 'name')

    const {data: relatives} = useGetAllQuery({
        key: KEYS.relatives, url: URLS.relatives
    })
    const relativeList = getSelectOptionsListFromData(get(relatives, `data`, []), 'id', 'name')
    ...
    const districtList = getSelectOptionsListFromData(get(district, `data`, []), 'id', 'name')
    ...
    return (<>
        ...
        <Section>
            ...
            <Form footer={!isEqual(get(data, 'data.status'), 'payed') && <Flex className={'mt-32'}>{(isEqual(get(data, 'data.status'), 'new') || isEqual(get(data, 'data.status'), 'edited')) && <><Button onClick={remove}
                                                                                                                                                                                                                                danger type={'button'}
                ...
                <Button onClick={(isEqual(get(data, 'data.status'),'new') || isEqual(get(data, 'data.status'),'edited')) ? () =>send() : ()=>{}} gray={!(isEqual(get(data, 'data.status'),'new') || isEqual(get(data, 'data.status'),'edited'))} type={'button'} className={'mr-16'}>Отправить в
                    Фонд</Button>
                ...
                <Row gutterWidth={60} className={'mt-32'}>
                    ...
                    <Col xs={4}>
                        ...
                        <Row align={'center'} className={'mb-25'}>
                            ...
                            <Col xs={7}><Field defaultValue={get(data, 'data.cost.sumInsured')}
                        ...
                        <Row align={'center'} className={'mb-25'}>
                            ...
                            <Col xs={7}><Field defaultValue={get(data, 'data.cost.insurancePremium')}
                                               property={{hideLabel: true, disabled: true}}
                                               type={'number-format-input'}
                ...
                <Row gutterWidth={60} className={'mt-30'}>
                    ...
                    <Col xs={4} style={{borderRight: '1px solid #DFDFDF'}}>
                        ...
                        <Row align={'center'} className={'mb-25'}>
                            ...
                            <Col xs={7}><Field defaultValue={get(data, 'data.vehicle.techPassport.seria')}
                                               property={{hideLabel: true, disabled: true}}
                                               type={'input'}
                        ...
                        <Row align={'center'} className={'mb-25'}>
                            ...
                            <Col xs={7}><Field defaultValue={get(data, 'data.vehicle.techPassport.number')}
                                               property={{hideLabel: true, disabled: true}}
                                               type={'input'}
                    ...
                    <Col xs={8}>
                        <Row>
                            <Col xs={4} className="mb-25">
                                <Field disabled defaultValue={get(data, 'data.vehicle.regionId')}
                                       options={regionList}
                                       params={{required: true}}
                            ...
                            <Col xs={4} className="mb-25">
                                <Field disabled defaultValue={get(data, 'data.vehicle.typeId')}
                            ...
                            <Col xs={4} className="mb-25">
                                <Field disabled defaultValue={get(data, 'data.vehicle.issueYear')}
                            ...
                            <Col xs={4} className="mb-25">
                                <Field disabled defaultValue={get(data, 'data.vehicle.techPassport.issueDate')}
                                       params={{required: true}}
                    ...
                    {get(data, 'data.owner.person') && <>
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                ...
                                defaultValue={get(data, 'data.owner.person.birthCountryCode')}
                                label={'Country'}
                                type={'input'}
                    ...
                    {isEqual(data, 'data.owner') && <>
                        <Col xs={3} className={'mb-25'}>
                            <Field disabled params={{required: true}} label={'INN'}
                                   defaultValue={get(data, 'data.owner.organization.inn')} property={{
                                mask: '999999999',
                                placeholder: 'Inn',
                                maskChar: '_'
                    ...
                    {get(data, 'data.applicant.person') && <>
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                params={{required: true}}
                                defaultValue={get(data, 'data.applicant.person.fullName.firstname')}
                                label={'Firstname'}
                                type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                params={{required: true}}
                                defaultValue={get(data, 'data.applicant.person.fullName.lastname')}
                                label={'Lastname'}
                                type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                params={{required: true}}
                                defaultValue={get(data, 'data.applicant.person.fullName.middlename')}
                                label={'Middlename'}
                                type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field params={{required: true}}
                                   property={{disabled: true}}
                                   defaultValue={get(data, 'data.applicant.person.passportData.pinfl')}
                                   label={'ПИНФЛ'} type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field disabled params={{required: true}} property={{
                                ...
                                placeholder: 'AA',
                                maskChar: '_'
                            }} defaultValue={get(data, 'data.applicant.person.passportData.seria')}
                                   label={'Passport seria'} type={'input-mask'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                ...
                                property={{
                                    ...
                                    placeholder: '1234567',
                                    maskChar: '_'
                                }} defaultValue={get(data, 'data.applicant.person.passportData.number')}
                                label={'Passport number'} type={'input-mask'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                disabled
                                defaultValue={get(data, 'data.applicant.person.passportData.issueDate')}
                                params={{required: true}}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field property={{disabled: true}} params={{required: true}}
                                   defaultValue={get(data, 'data.applicant.person.passportData.issuedBy')}
                                   label={'Issued by'}
                                   type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                defaultValue={get(data, 'data.applicant.person.birthCountryCode')}
                                label={'Country'}
                                type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                disabled
                                options={residentTypeList}
                                defaultValue={get(data, 'data.applicant.person.residentType')}
                                label={'Resident type'}
                                type={'select'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                defaultValue={get(data, 'data.applicant.person.phoneNumber')}
                                label={'Phone'}
                                type={'input'}
                    ...
                    {get(data, 'data.applicant.organization') && <>
                        <Col xs={3} className={'mb-25'}>
                            <Field params={{required: true}} label={'INN'}
                                   defaultValue={get(data, 'data.applicant.person.inn')} property={{
                                mask: '999999999',
                                placeholder: 'Inn',
                                maskChar: '_'
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                defaultValue={get(data, 'data.applicant.organization.phone')}
                                params={{required: true}}
                                label={'Телефон'} type={'input'}
                        ...
                        <Col xs={3} className={'mb-25'}>
                            <Field
                                property={{disabled: true}}
                                defaultValue={get(data, 'data.applicant.organization.email')}
                    ...
                    <Col xs={12}>
                        <div className={'horizontal-scroll '}>
                            <Table bordered hideThead={false}
                                ...
                                {
                                    get(data,'data.drivers',[]).map((item, index) => <tr>
                ...
                <Row gutterWidth={60} className={'mt-30'}>
                    ...
                    <Col xs={6}>
                        <Row>
                            ...
                            <Col xs={6} className={'mb-25'}>
                                <Field
                                    defaultValue={round(get(data,'data.agentReward',20) * get(data,'data.cost.insurancePremium',0) / 100, 2)}
                                    property={{disabled: true}}
                                    label={'Сумма'}
                                    type={'number-format-input'}
                                    name={'rewardSum'}/>
                            ...
                            <Col xs={6} className={'mb-25'}>
                                <Field
                                    defaultValue={round(5 * get(data,'data.cost.insurancePremium',0)  / 100, 2)}
                                    property={{disabled: true}}
                                    label={'Сумма'}
                                    type={'number-format-input'}
                                    name={'rpmSum'}/>
